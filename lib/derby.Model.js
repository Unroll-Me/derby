...
function init(derby, app) {
  ...
  ['connected', 'canConnect'].forEach(function(event) {
    model.listeners(event).unshift(function handler(value) {
      // If the connection breaks during a model update,
      // raise this event after we finish the update, to
      // make sure that the UI is properly updated.
      if (dom._preventUpdates) {
        dom._pendingUpdates.push(handler.bind(this, value));
        return;
      }
  ...
  model.on('bulkUpdate', function () {
    if (dom._preventUpdates)
      return;
    setTimeout(function () {
      app.view.render(model, app.view._lastRender.ns, app.view._lastRender.context);
    }, 10); // Wait for all bulkUpdate handlers to run before re-rendering.
  });
