...
function escapeInlineScript(s) {
  // U+2028 LINE SEPARATOR is valid in JSON string literals but not Javascript string literals
...
View.prototype.render = function(res) {
  ...
  if (model == null) {
    model = Object.create(emptyModel);
    model.req = res.req;
  }
...
View.prototype.pack = function(isStatic, callback) {
  // Old signature: pack(callback);
  if (arguments.length === 1 && typeof isStatic === 'function') {
    callback = isStatic;
    isStatic = false;
  }
  ...
  view._loadStyles(root, clientName, function(err, css) {
    ...
    view._loadTemplates(root, clientName, function(err, templates, instances, libraryData) {
      ...
      if (isStatic) {
        ...
        return files.writeGen(root, filename, file, true, finish);
      }
...
function loadAll(view, isStatic, callback) {
  ...
  if (isProduction) {
    try {
      ...
      if (data.inline) view.inline("function(){" + data.inline + "}");
